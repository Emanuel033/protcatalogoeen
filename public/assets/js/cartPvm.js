...
window.renderCart = function() {
    ...
    if (window.cart.length === 0) {
        ...
        btnPreparar.disabled = true; btnPreparar.classList.add('opacity-50', 'cursor-not-allowed'); return;
    }

    btnPreparar.disabled = false; btnPreparar.classList.remove('opacity-50', 'cursor-not-allowed');
    container.innerHTML = ''; 
    let subtotalCentavos = 0; // Matemática exacta

    window.cart.forEach((item, index) => {
        const itemSubtotalCentavos = window.toCentavos(item.precio) * item.qty; 
        ...
        container.innerHTML += `
            <div class="bg-slate-50 border border-slate-100 p-2 md:p-3 rounded-xl flex flex-col relative animate-[fadeIn_0.2s_ease-out]">
                <div class="flex justify-between items-start mb-2">
                    ...
                    <button onclick="window.updateCartQty(${index}, -${item.qty})" class="absolute top-1.5 md:top-2 right-1.5 md:right-2 text-slate-300 hover:text-red-500"><i class="fas fa-times text-sm"></i></button>
                </div>
                <div class="flex justify-between items-end mt-auto">
                    <div class="flex items-center bg-white border border-slate-200 rounded-lg p-0.5 shadow-sm">
                        ...
                        <input type="text" inputmode="numeric" pattern="[0-9]*" autocomplete="off" data-lpignore="true" data-1p-ignore="true" data-form-type="other" spellcheck="false" value="${item.qty}" 
                    ...
                    <div class="text-right">
                        <div class="text-[8px] md:text-[9px] text-slate-400 font-bold">${window.formatMoney(item.precio)} c/u</div>
...
window.manualCartQtyVisual = function(index, val, el) {
    ...
    const itemSubtotalCentavos = window.toCentavos(window.cart[index].precio) * num;
    ...
    window.cart.forEach(x => subtotalCentavos += (window.toCentavos(x.precio) * (x.qty || 1)));
    const ivaCentavos = Math.round(subtotalCentavos * 0.16);
    const totalCentavos = subtotalCentavos + ivaCentavos;
...
window.openCheckoutModal = function() {
    if (window.cart.length === 0) return;
    let subtotalCentavos = 0;
    window.cart.forEach(item => { subtotalCentavos += window.toCentavos(item.precio) * (item.qty || 1); });
...
window.processSale = async function() {
    ...
    if (!navigator.onLine) {
        window.showToast("Estás sin conexión a internet. Espera a que regrese la red para cobrar.", "error");
        return;
    }
    ...
    let total = Number(window.currentTotalToPay);
    ...
    try {
        ...
        window.showToast("Ocurrió un error al guardar la venta: " + error.message, "error"); 
    } finally {
...
window.printTicketHTML = function(venta) {
    // La lógica visual del ticket sigue igual, no requiere muchos cambios.
    const printArea = document.getElementById('print-ticket');
    let fechaObj = new Date();
    if (venta.fecha) {
        if (typeof venta.fecha.toDate === 'function') fechaObj = venta.fecha.toDate();
        else if (venta.fecha instanceof Date) fechaObj = venta.fecha;
    }
        ...
        <div class="ticket-copy">
            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 10px; font-family: 'Courier New', Courier, monospace;">
                <div style="width: 65%; display: flex; gap: 8px; align-items: flex-start;">
                    <img src="https://i.ibb.co/dJr7BzL5/logoeen.png" style="height: 45px; width: auto; max-width: 80px; object-fit: contain; margin-top: 2px;">
                    <div>
                        <h2 style="margin:0; font-size: 12px; font-weight: 900; line-height: 1.1;">ENVASES LA ECONÓMICA DEL NORTE</h2>
                        <p style="margin:4px 0 0 0; font-size: 9px; line-height: 1.2;">Calzada Guadalupe Victoria 105 Oriente,<br>Col. Obrerista, Monterrey, N.L. C.P. 64470</p>
                    </div>
                </div>
                <div style="width: 35%; text-align: right;">
                    ...
                    <p style="margin:2px 0; font-size: 10px; font-weight: bold;">Folio: ${venta.folio}</p>
                    <p style="margin:2px 0; font-size: 10px;">${fechaStr} - ${horaStr}</p>
                    <p style="margin:2px 0; font-size: 9px; font-weight: bold;">Cajero: ${venta.vendedor || 'General'}</p>
                </div>
            </div>
        ...
        html += `</table>
            <div style="border-top: 1px solid #000; padding-top: 8px; font-family: 'Courier New', Courier, monospace; font-size: 11px; display: flex; justify-content: flex-end;">
                <div style="text-align: right; width: 200px;">
                    <div style="display: flex; justify-content: space-between;"><span style="color: #666;">Subtotal:</span> <span>${window.formatMoney(subtotal)}</span></div>
                    <div style="display: flex; justify-content: space-between;"><span style="color: #666;">IVA (16%):</span> <span>${window.formatMoney(iva)}</span></div>
                    <div style="display: flex; justify-content: space-between; border-top: 1px solid #000; margin-top: 4px; padding-top: 4px; font-weight: 900; font-size: 13px;"><span>TOTAL:</span> <span>${window.formatMoney(venta.total)}</span></div>
                    ${infoPago}
                </div>
            </div>`;

        if (copy.signType === 'client') { html += `<div style="margin-top: 30px; margin-bottom: 10px; text-align: center; width: 100%; font-family: 'Courier New', Courier, monospace;"><div style="border-top: 1px solid #000; width: 70%; margin: 0 auto; padding-top: 8px; font-weight: bold; font-size: 11px;">Nombre y Firma de Recibido / Conformidad</div></div>`; } 
        else if (copy.signType === 'warehouse') { html += `<div style="margin-top: 30px; margin-bottom: 10px; display: flex; justify-content: space-between; width: 100%; font-family: 'Courier New', Courier, monospace;"><div style="text-align: center; width: 45%;"><div style="border-top: 1px solid #000; padding-top: 8px; font-weight: bold; font-size: 11px;">Surtió</div></div><div style="text-align: center; width: 45%;"><div style="border-top: 1px solid #000; padding-top: 8px; font-weight: bold; font-size: 11px;">Revisó</div></div></div>`; } 
        else { html += `<div style="margin-top: 10px;"></div>`; }

        html += `<div style="margin-top: 10px; text-align: center; font-family: 'Courier New', Courier, monospace; font-size: 9px; line-height: 1.4; border-top: 1px dashed #000; padding-top: 10px; color: #111;"><div style="font-weight: bold; font-size: 10px;">TELÉFONOS OFICINA</div><div>81 8375 4630 &bull; 81 8372 8736 &bull; 81 8374 6703 &bull; 81 8375 1518</div><div style="font-weight: bold; font-size: 10px; margin-top: 5px;">WHATSAPP VENTAS</div><div>81 1372 8493 &bull; 81 1840 0503</div><div style="margin-top: 5px;"><b>Correo:</b> ventas@laeconomicamty.com</div><div style="margin-top: 10px; display: flex; justify-content: center; align-items: center; gap: 10px;"><img src="https://api.qrserver.com/v1/create-qr-code/?size=60x60&data=https://productoseen.web.app" style="width: 45px; height: 45px; border: 1px solid #ccc; padding: 2px;"><div style="text-align: left;"><div style="font-weight: 900; font-size: 10px;">¡Escanea y compra en línea!</div><div style="font-size: 10px;">productoseen.web.app</div></div></div><div style="margin-top: 12px; font-weight: bold; font-size: 11px;">¡GRACIAS POR SU PREFERENCIA!</div></div></div>`;
    });
    
    printArea.innerHTML = html; 
    requestAnimationFrame(() => { requestAnimationFrame(() => { try { window.print(); } catch(e) { console.error(e); window.showToast("Venta guardada. Error al imprimir.", "error"); } }); });
}
