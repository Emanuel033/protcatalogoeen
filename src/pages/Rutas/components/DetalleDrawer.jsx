...
const DetalleDrawer = ({ pedidoSeleccionado, onClose, onEdit }) => {
  ...
  return (
    <>
      ...
      {/* FONDO MAESTRO DEL DRAWER (CRISTAL ESMERILADO CLARO) */}
      <div className={`fixed bottom-0 lg:top-4 lg:bottom-4 right-0 lg:right-4 w-full lg:w-[380px] h-[75vh] lg:h-[calc(100vh-2rem)] bg-slate-100/50 backdrop-blur-2xl lg:rounded-3xl rounded-t-3xl shadow-[0_10px_40px_rgba(0,0,0,0.2)] z-[40] flex flex-col overflow-hidden border border-white/50 transform transition-transform duration-300 ease-out ${isOpen ? 'translate-y-0 lg:translate-x-0' : 'translate-y-full lg:translate-x-[120%]'}`}>
        
        {/* HEADER AZUL CON ALTA LEGIBILIDAD */}
        <div className="bg-blue-600/95 backdrop-blur-sm p-4 shrink-0 relative shadow-md z-10">
          ...
          <div className="flex justify-between items-start mb-1 pr-8">
            <div className="flex gap-1.5 items-center">
              {pedidoSeleccionado.folio_pedido && (<span className="text-[9px] font-mono font-bold text-blue-900 bg-white/90 px-1.5 py-0.5 rounded border border-white shadow-sm">PED: {pedidoSeleccionado.folio_pedido}</span>)}
              ...
              <button onClick={handleEliminar} className="text-red-100 hover:text-white bg-red-600/40 px-1.5 py-0.5 rounded text-[9px] font-bold transition flex items-center gap-1 ml-1 border border-red-500/50"><i className="fas fa-trash"></i></button>
            </div>
          </div>
          
          <h3 className="text-lg font-black text-white leading-tight mt-1 truncate">
            {pedidoSeleccionado.cliente_nombre}
            {esContpaqi && <span className="bg-slate-800 text-white px-1.5 py-0.5 rounded text-[8px] font-black uppercase align-middle ml-2 shadow-sm border border-slate-700">CONTPAQI</span>}
            {pedidoSeleccionado.urgente && <span className="bg-red-500 text-white px-1.5 py-0.5 rounded text-[8px] font-black uppercase align-middle ml-2 shadow-sm"><i className="fas fa-fire-alt"></i> Urgente</span>}
          </h3>
        ...
        <div className="p-3 overflow-y-auto custom-scroll flex-1 min-h-0 space-y-3 pb-6 relative z-0">
          ...
          {/* ACORDEÓN: INFO DE ENTREGA (CRISTAL BLANCO + TEXTO OSCURO PARA MÁXIMA LEGIBILIDAD) */}
          <div className="bg-white/85 backdrop-blur-md border border-white/60 rounded-xl shadow-sm overflow-hidden shrink-0">
            <button onClick={() => setSeccionInfo(!seccionInfo)} className="w-full flex justify-between items-center p-3 hover:bg-white/50 transition">
            ...
            {seccionInfo && (
              <div className="p-3 pt-0 border-t border-slate-200/50">
                <div className="bg-slate-100/80 p-2 rounded-lg mb-2 border border-slate-200 mt-2">
                ...
                <div className="flex flex-col gap-1.5 mt-3 mb-3">
                    <div className="flex justify-between items-center text-[10px] font-bold text-slate-700 bg-white border border-slate-200 p-2 rounded-lg">
                        <span><i className="fas fa-user text-slate-400 w-4 text-center"></i> Cliente: {pedidoSeleccionado.telefono_contacto || 'S/N'}</span>
                        <button className="text-blue-600 hover:text-blue-800 transition"><i className="fas fa-phone"></i></button>
                    </div>
                    <div className="flex justify-between items-center text-[10px] font-bold text-slate-700 bg-white border border-slate-200 p-2 rounded-lg">
                ...
                {/* AVISO DE COBRANZA - ALTO CONTRASTE RESTAURADO */}
                {requiereCobro && (
                  <div className="bg-red-100/90 backdrop-blur-md border border-red-200 rounded-xl p-3 mb-3 flex justify-between items-center shadow-sm">
                     <div className="flex items-start gap-2">
                        ...
                        <div>
                           ...
                           <p className="text-[9px] font-medium text-red-700 leading-snug">Adeudo reportado de CONTPAQi</p>
                        </div>
                     </div>
                     <span className="text-sm font-black text-red-800">${saldo.toFixed(2)}</span>
                  </div>
                )}
                ...
                <div className="mt-3 pt-2 border-t border-slate-200/50">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {docs.factura && <span className="bg-blue-100 text-blue-800 text-[9px] font-bold px-2 py-1 rounded-md border border-blue-200 shadow-sm"><i className="fas fa-file-invoice mr-1"></i>Factura</span>}
                    {docs.certificados && <span className="bg-amber-100 text-amber-800 text-[9px] font-bold px-2 py-1 rounded-md border border-amber-200 shadow-sm"><i className="fas fa-certificate mr-1"></i>Certificados</span>}
          ...
          {/* ACORDEÓN: TRAYECTO ESTIMADO (CRISTAL BLANCO + ALTA LEGIBILIDAD) */}
          {!esPendiente && (
              <div className="bg-indigo-50/85 backdrop-blur-md border border-indigo-200/60 rounded-xl shadow-sm overflow-hidden shrink-0">
                <button onClick={() => setSeccionTrayecto(!seccionTrayecto)} className="w-full flex justify-between items-center p-3 hover:bg-indigo-100/50 transition">
                  <h4 className="font-black text-[11px] text-indigo-900 flex items-center gap-1.5"><i className="fas fa-route text-indigo-600"></i> Trayecto Estimado</h4>
                  <div className="flex items-center gap-2">
                    {esRampa && (
                      <span onClick={(e) => { e.stopPropagation(); handleOptimizar(); }} className="text-[9px] font-bold text-indigo-700 hover:text-indigo-900 bg-indigo-200/50 px-2 py-1 rounded transition flex items-center gap-1">
                        <i className="fas fa-magic"></i> Optimizar
                      </span>
                    )}
                    <i className={`fas fa-chevron-${seccionTrayecto ? 'up' : 'down'} text-indigo-500 text-xs transition-transform`}></i>
                  </div>
                </button>
                
                {seccionTrayecto && (
                  <div className="p-3 pt-0 border-t border-indigo-200/50">
                    ...
                    {paradasRuta.map((parada, index) => {
                        ...
                        return (
                        <div key={parada.id} className={`flex items-center gap-1.5 py-1.5 relative border-l-2 ml-2.5 pl-3 transition-colors ${isSelectedParada ? 'bg-indigo-200/50 border-indigo-500' : 'border-indigo-200 bg-transparent'}`}>
                          {parada.tipo === 'destino' && <div className="drag-handle w-4 h-full flex items-center justify-center text-slate-400 hover:text-indigo-600 cursor-grab active:cursor-grabbing"><i className="fas fa-grip-vertical"></i></div>}
                          <div className={`w-4 h-4 text-white rounded-full flex items-center justify-center text-[8px] font-bold shadow-sm shrink-0 ${parada.tipo === 'origen' ? 'bg-slate-800 absolute -left-[11px]' : parada.tipo === 'retorno' ? 'bg-emerald-600 absolute -left-[11px]' : isSelectedParada ? 'bg-indigo-600 absolute -left-[11px] scale-110 shadow-indigo-500/50' : 'bg-blue-500 absolute -left-[11px]'}`}>
                              {parada.tipo === 'origen' ? <i className="fas fa-industry"></i> : parada.tipo === 'retorno' ? <i className="fas fa-flag-checkered"></i> : index}
                          </div>
                          <span className={`truncate text-[10px] font-bold ${isSelectedParada ? 'text-indigo-900' : 'text-slate-700'}`}>{parada.nombre}</span>
                        </div>
                    )})}
                    </ReactSortable>

                    <div className="mt-3 pt-3 border-t border-indigo-200/50 flex justify-around bg-white/80 rounded-lg p-2 shadow-sm">
                        <div className="text-center">
                            <p className="text-[8px] font-black text-slate-500 uppercase tracking-wide">Distancia</p>
                            <p className="text-xs font-bold text-indigo-800">{distanciaRuta} km</p>
                        </div>
                        <div className="w-px bg-indigo-200"></div>
                        <div className="text-center">
                            <p className="text-[8px] font-black text-slate-500 uppercase tracking-wide">Tiempo</p>
                            <p className="text-xs font-bold text-emerald-700">{tiempoRuta} min</p>
                        </div>
                    </div>
                  </div>
                )}
          ...
          {(esRampa || esEnRuta) && !modoEdicionAsignacion && (
             <div className="bg-white/90 backdrop-blur-md border border-slate-200 rounded-xl p-3 shadow-sm flex justify-between items-center shrink-0">
                 <div>
                     <p className="text-[9px] font-black text-slate-500 uppercase mb-1">Unidad y Operador Asignado:</p>
                     <p className="text-[11px] font-bold text-blue-800"><i className="fas fa-truck text-blue-500 w-4"></i> {flota.find(v => v.id === vehiculoId)?.nombre || 'S/N'}</p>
                     <p className="text-[11px] font-bold text-slate-800"><i className="fas fa-user-circle text-slate-500 w-4"></i> {choferes.find(c => c.id === choferId)?.nombre || 'S/N'}</p>
                 </div>
                 <button onClick={() => setModoEdicionAsignacion(true)} className="w-8 h-8 rounded-full bg-slate-100 border border-slate-200 hover:bg-slate-200 text-slate-700 flex items-center justify-center transition shadow-sm">
                     <i className="fas fa-pencil-alt text-xs"></i>
                 </button>
             </div>
          )}

          {/* ASIGNACIÓN DE UNIDAD (CAJA OSCURA DE ALTO CONTRASTE) */}
          {(esPendiente || modoEdicionAsignacion) && (
             <div className="bg-slate-900/90 backdrop-blur-xl rounded-2xl p-4 shadow-xl text-white border border-slate-700 relative shrink-0 mt-3">
                {modoEdicionAsignacion && (
                    <button onClick={() => { setModoEdicionAsignacion(false); setVehiculoId(pedidoSeleccionado.vehiculo_asignado); setChoferId(pedidoSeleccionado.chofer_asignado); }} className="absolute top-3 right-3 text-slate-400 hover:text-white bg-white/10 rounded-full w-6 h-6 flex items-center justify-center transition">
                        <i className="fas fa-times"></i>
                    </button>
                )}

                <h4 className="text-[10px] font-black uppercase tracking-wider mb-3 text-slate-300">
                ...
                <div className="grid grid-cols-3 gap-2 mb-4">
                  {flota.map(v => {
                    ...
                    const iconColor = v.pesado ? 'text-blue-400' : 'text-slate-400';
                    return (
                      <button key={v.id} onClick={() => setVehiculoId(v.id)} className={`p-2 rounded-xl border text-[9px] font-bold transition-all text-center flex flex-col items-center gap-1.5 ${isSelected ? 'border-blue-400 bg-blue-600 text-white shadow-inner scale-105' : 'border-slate-600 bg-slate-800 text-slate-300 hover:border-slate-500 hover:bg-slate-700'}`}>
                ...
                <div className="relative mb-3">
                  ...
                  <select value={choferId} onChange={(e) => setChoferId(e.target.value)} className="w-full border border-slate-600 bg-slate-800 text-white p-3 pl-9 rounded-xl outline-none text-xs font-bold cursor-pointer shadow-inner appearance-none transition focus:border-blue-500">
                ...
                {advertenciaChofer && !errorEmpalme && (
                    <div className="mb-4 bg-amber-500/20 border border-amber-500/50 rounded-xl p-2.5 flex items-start gap-2 shadow-sm">
                        <i className="fas fa-info-circle text-amber-400 mt-0.5"></i>
                        <p className="text-[10px] text-amber-100 font-medium leading-snug">Este chofer ya tiene pedidos en OTRA unidad.</p>
                    </div>
                )}

                {errorEmpalme && (
                    <div className="mb-4 bg-red-500/20 border border-red-500/50 rounded-xl p-2.5 flex items-start gap-2 shadow-sm">
                        <i className="fas fa-hand-paper text-red-400 mt-0.5"></i>
                        <p className="text-[10px] text-red-100 font-medium leading-snug"><b>Empalme:</b> {errorEmpalme}</p>
                    </div>
                )}
                
                {esPendiente ? (
                    <button onClick={() => cambiarEstadoLogistico('rampa')} disabled={!!errorEmpalme} className={`w-full font-black py-3 rounded-xl shadow-lg transition flex items-center justify-center gap-2 text-xs ${errorEmpalme ? 'bg-slate-700 text-slate-500 border border-slate-600 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-500 active:scale-95'}`}>
                        <i className="fas fa-link"></i> Pre-Asignar
                    </button>
                ) : (
                    <button onClick={() => cambiarEstadoLogistico('actualizar_asignacion')} disabled={!!errorEmpalme} className={`w-full font-black py-3 rounded-xl shadow-lg transition flex items-center justify-center gap-2 text-xs ${errorEmpalme ? 'bg-slate-700 text-slate-500 border border-slate-600 cursor-not-allowed' : 'bg-emerald-600 hover:bg-emerald-500 text-white border border-emerald-500 active:scale-95'}`}>
          ...
          {/* CONFIRMAR SALIDA (ALTA LEGIBILIDAD) */}
          {esRampa && !modoEdicionAsignacion && (
             <div className="bg-emerald-100/90 backdrop-blur-md border border-emerald-200 rounded-2xl p-4 shadow-sm mt-2 relative overflow-hidden">
                 <h4 className="text-[11px] font-black text-emerald-800 flex items-center gap-1.5 mb-2 relative z-10"><i className="fas fa-truck-fast text-emerald-600"></i> CONFIRMAR SALIDA</h4>
                 <p className="text-[10px] text-emerald-700 mb-3 leading-snug relative z-10 font-medium">Presiona aquí cuando arranque para registrar la hora exacta de salida y crear el Lote de Viaje.</p>
                 <button onClick={() => cambiarEstadoLogistico('salida')} className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-black py-3 rounded-xl shadow-lg transition active:scale-95 flex items-center justify-center gap-2 text-xs relative z-10">
          ...
          {(esFallido || esEntregado) && (
              <div className="bg-white/85 backdrop-blur-md border border-white/60 rounded-xl shadow-sm p-3 mt-3">
                  <h4 className="font-black text-[11px] text-slate-800 flex items-center gap-1.5 mb-2"><i className="fas fa-camera text-blue-600"></i> Evidencias y Reportes</h4>
                  <div className="text-center text-[10px] text-slate-500 py-3 border border-dashed border-slate-300 rounded-lg mb-2 bg-slate-50 shadow-inner">Sin fotos adjuntas</div>
                  {esFallido && (
                      <div className="bg-red-50/90 backdrop-blur-sm border border-red-200 p-3 rounded-lg mt-2 shadow-sm">
                          <span className="text-[10px] font-black text-red-800 flex items-center gap-1 mb-1"><i className="fas fa-exclamation-triangle text-red-600"></i> MOTIVO DE FALLA</span>
                          <p className="text-xs font-bold text-red-900 mb-3 leading-snug">{pedidoSeleccionado.motivo_falla || 'Falla reportada por el chofer.'}</p>
                          <button onClick={() => cambiarEstadoLogistico('reasignar')} className="w-full bg-amber-500 text-white text-[11px] font-black py-2.5 rounded-xl shadow-md hover:bg-amber-400 transition flex items-center justify-center gap-2"><i className="fas fa-redo"></i> Reasignar a Pendientes</button>
                      </div>
                  )}
              </div>
          )}
        ...
        {/* BOTONES FLOTANTES */}
        {esEnRuta && (
             <div className="absolute bottom-0 left-0 w-full bg-white/80 backdrop-blur-md border-t border-slate-200 p-3 shadow-[0_-10px_30px_rgba(0,0,0,0.1)] flex gap-2 z-20">
                 <button onClick={() => cambiarEstadoLogistico('entregado')} className="flex-[2] bg-emerald-600 text-white text-[11px] font-black py-3 rounded-xl shadow-lg hover:bg-emerald-500 transition active:scale-95 flex items-center justify-center gap-2"><i className="fas fa-check-double text-emerald-100"></i> Marcar Entregado</button>
                 <button onClick={() => cambiarEstadoLogistico('fallido')} className="flex-1 bg-white text-amber-700 border border-amber-200 text-[10px] font-black py-3 rounded-xl hover:bg-amber-50 transition flex items-center justify-center gap-1 shadow-sm"><i className="fas fa-exclamation-triangle text-amber-500"></i> Incidencia</button>
             </div>
        )}
      </div>
    </>
