    ...
    <>
      ...
      <div className={`fixed bottom-0 lg:top-4 lg:bottom-4 right-0 lg:right-4 w-full lg:w-[380px] h-[85vh] lg:h-[calc(100vh-2rem)] bg-white/95 sm:bg-white/40 backdrop-blur-2xl lg:rounded-3xl rounded-t-3xl shadow-2xl z-[40] flex flex-col overflow-hidden border border-white/40 transform-gpu transition-transform duration-300 ease-out ${isOpen ? 'translate-y-0' : 'translate-y-full lg:translate-x-[120%]'}`}>
        ...
        <div className="p-3 overflow-y-auto custom-scroll flex-1 min-h-0 space-y-3 pb-6 relative z-0">
            ...
            {seccionInfo && (
              <div className="p-3 pt-0 border-t border-white/10">
                ...
                <div className="flex flex-col gap-1.5 mt-3 mb-3">
                    <div className="flex justify-between items-center text-[10px] font-bold text-slate-800 bg-white/30 border border-white/20 p-2 rounded-lg shadow-sm">
                        <span><i className="fas fa-user text-blue-600 w-4 text-center"></i> Cliente: {pedidoSeleccionado.telefono_contacto || 'S/N'}</span>
                        <button className="text-blue-600 hover:text-blue-800 transition"><i className="fas fa-phone"></i></button>
                    </div>
                    <div className="flex justify-between items-center text-[10px] font-bold text-slate-800 bg-white/30 border border-white/20 p-2 rounded-lg shadow-sm">
                        <span><i className="fas fa-warehouse text-slate-600 w-4 text-center"></i> Destino: {pedidoSeleccionado.destino_telefono || 'S/N'}</span>
                        <button className="text-blue-600 hover:text-blue-800 transition"><i className="fas fa-phone"></i></button>
                    </div>
                </div>
                ...
                <div className="mt-3 pt-2 border-t border-white/10">
                  <p className="text-[9px] font-black text-slate-600 uppercase mb-1">Entregar con:</p>
                  <div className="flex flex-wrap gap-1.5">
                    {docs.factura && <span className="bg-blue-100/80 text-blue-900 text-[9px] font-bold px-2 py-1 rounded-md border border-blue-300/50 backdrop-blur-sm shadow-sm"><i className="fas fa-file-invoice mr-1"></i>Factura</span>}
                    {docs.certificados && <span className="bg-amber-100/80 text-amber-900 text-[9px] font-bold px-2 py-1 rounded-md border border-amber-300/50 backdrop-blur-sm shadow-sm"><i className="fas fa-certificate mr-1"></i>Certificados</span>}
                    {docs.envio_ciego && <span className="bg-slate-800/70 text-white text-[9px] font-bold px-2 py-1 rounded-md border border-slate-600/50 backdrop-blur-sm shadow-sm"><i className="fas fa-user-secret mr-1"></i>Envío Ciego</span>}
                    {(!docs.factura && !docs.certificados && !docs.envio_ciego) && <span className="text-[9px] text-slate-500 font-bold">Sin requisitos</span>}
                  </div>
                </div>
              </div>
            )}
          ...
          {!esPendiente && (
              <div className="bg-indigo-50/50 border border-indigo-200/50 backdrop-blur-sm rounded-xl shadow-sm overflow-hidden shrink-0">
                <button onClick={() => setSeccionTrayecto(!seccionTrayecto)} className="w-full flex justify-between items-center p-3 hover:bg-white/30 transition">
                  ...
                  <div className="flex items-center gap-2">
                    {esRampa && (
                      <span onClick={(e) => { e.stopPropagation(); handleOptimizar(); }} className="text-[9px] font-bold text-indigo-700 hover:text-indigo-900 bg-white/50 px-2 py-1 rounded transition flex items-center gap-1 border border-white/60">
                ...
                {seccionTrayecto && (
                  <div className="p-3 pt-0 border-t border-indigo-200/30">
                    ...
                    {paradasRuta.map((parada, index) => {
                        ...
                        return (
                        <div key={parada.id} className={`flex items-center gap-1.5 py-1.5 relative border-l-2 ml-2.5 pl-3 transition-colors ${isSelectedParada ? 'bg-white/40 border-indigo-500' : 'border-indigo-200/50 bg-transparent'}`}>
                    ...
                    <div className="mt-3 pt-3 border-t border-indigo-200/30 flex justify-around bg-white/40 backdrop-blur-sm rounded-lg p-2 shadow-sm border border-white/50">
          ...
          {(esRampa || esEnRuta) && !modoEdicionAsignacion && (
             <div className="bg-white/30 backdrop-blur-sm border border-white/40 rounded-xl p-3 shadow-sm flex justify-between items-center shrink-0">
                 <div>
                     <p className="text-[9px] font-black text-slate-600 uppercase mb-1">Unidad y Operador Asignado:</p>
                 ...
                 <button 
                    ...
                    className="w-8 h-8 rounded-full bg-white/40 border border-white/50 hover:bg-white/60 text-slate-700 flex items-center justify-center transition shadow-sm"
                 >
          ...
          {(esPendiente || modoEdicionAsignacion) && (
             <div className="bg-blue-900/80 backdrop-blur-xl rounded-2xl p-5 border border-blue-400/30 text-white shadow-xl">
                ...
                <h4 className="text-[10px] font-black uppercase tracking-wider mb-3 text-white drop-shadow-sm">
                ...
                <div className="grid grid-cols-3 gap-2 mb-4">
                  {flota.map(v => {
                    ...
                    return (
                      <button key={v.id} onClick={() => setVehiculoId(v.id)} className={`p-2 rounded-xl border text-[9px] font-bold transition-all text-center flex flex-col items-center gap-1.5 ${isSelected ? 'border-white bg-white/30 backdrop-blur-sm text-white shadow-inner scale-105' : 'border-white/20 bg-black/10 backdrop-blur-sm text-blue-100 hover:border-white/40 hover:bg-white/10'}`}>
                ...
                <div className="relative mb-3">
                  ...
                  <select value={choferId} onChange={(e) => setChoferId(e.target.value)} className="w-full border border-white/30 bg-black/10 backdrop-blur-md text-white p-3 pl-9 rounded-xl outline-none text-xs font-bold cursor-pointer shadow-inner appearance-none transition focus:border-white">
                ...
                {advertenciaChofer && !errorEmpalme && (
                    <div className="mb-4 bg-amber-500/80 backdrop-blur-sm border border-amber-300 rounded-xl p-2.5 flex items-start gap-2 shadow-sm">
                        <i className="fas fa-info-circle text-white mt-0.5"></i>
                        <p className="text-[10px] text-white font-medium leading-snug drop-shadow-sm">
                ...
                {errorEmpalme && (
                    <div className="mb-4 bg-red-500/80 backdrop-blur-sm border border-red-300 rounded-xl p-2.5 flex items-start gap-2 shadow-sm">
                ...
                {esPendiente ? (
                    ...
                    <button onClick={() => cambiarEstadoLogistico('actualizar_asignacion')} disabled={!!errorEmpalme} className={`w-full font-black py-3 rounded-xl shadow-lg transition flex items-center justify-center gap-2 text-xs ${errorEmpalme ? 'bg-black/20 text-white/50 border border-white/20 cursor-not-allowed' : 'bg-emerald-500 hover:bg-emerald-400 text-white border border-emerald-300 active:scale-95'}`}>
          ...
          {(esFallido || esEntregado) && (
              <div className="bg-white/20 backdrop-blur-sm border border-white/30 rounded-xl shadow-inner p-3 mt-3">
                  ...
                  <div className="text-center text-[10px] text-slate-600 py-3 border border-dashed border-white/40 rounded-lg mb-2 bg-white/30 shadow-sm">Sin fotos adjuntas</div>
                  {esFallido && (
                      <div className="bg-red-500/40 backdrop-blur-md border border-red-400/60 p-3 rounded-lg mt-2 shadow-md">
                          ...
                          <p className="text-xs font-bold text-white mb-3 leading-snug drop-shadow-sm">{pedidoSeleccionado.motivo_falla || 'Falla reportada por el chofer.'}</p>
        ...
        {esEnRuta && (
             <div className="absolute bottom-0 left-0 w-full bg-white/50 backdrop-blur-xl border-t border-white/40 p-3 shadow-[0_-10px_30px_rgba(0,0,0,0.1)] flex gap-2 z-20">
                 <button onClick={() => cambiarEstadoLogistico('entregado')} className="flex-[2] bg-emerald-600/90 backdrop-blur-sm text-white text-[11px] font-black py-3 rounded-xl shadow-lg shadow-emerald-500/30 hover:bg-emerald-500 transition active:scale-95 flex items-center justify-center gap-2 border border-emerald-400/50"><i className="fas fa-check-double text-emerald-100"></i> Marcar Entregado</button>
                 <button onClick={() => cambiarEstadoLogistico('fallido')} className="flex-1 bg-white/40 backdrop-blur-sm text-red-900 border border-white/50 text-[10px] font-black py-3 rounded-xl hover:bg-white/60 transition flex items-center justify-center gap-1 shadow-sm"><i className="fas fa-exclamation-triangle text-red-600"></i> Incidencia</button>
             </div>
        )}
