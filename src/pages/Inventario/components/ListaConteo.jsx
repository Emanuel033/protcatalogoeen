          ...
          <div key={item.codigo} className={`bg-slate-800 p-4 rounded-3xl border ${borderColor} transition-colors shadow-lg`}>
            ...
            <div className="flex justify-between items-start mb-4">
              <div className="flex-1 pr-4">
                <h3 className="font-black text-lg text-white leading-tight">{item.nombre}</h3>
                <p className="font-mono text-xs text-blue-400 mt-1">{item.codigo}</p>
              </div>
              <button onClick={() => onEliminar(item.codigo)} className="text-slate-500 hover:text-red-400 p-2">
                ...
                return (
                  <div key={v.id} className="flex items-center justify-between py-2 border-b border-slate-700/50 last:border-0">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      ...
                      <button onClick={(e) => onIniciarDictado(item.codigo, v.id, e.currentTarget, letterIndex)} className="w-9 h-9 rounded-lg bg-slate-800 text-slate-400 border border-slate-700"><i className="fas fa-microphone text-sm"></i></button>
